...
class RecipeCrudModalNotExtended extends React.Component {
	...
	onSubmit = () => {
		...
		if (
			...
			formValues.ingredients = formValues.ingredients.replace(
				/(tablespoons)|(tablespoon)/gi,
				'Tbsp.'
			);
			formValues.ingredients = formValues.ingredients.replace(
				/(teaspoons)|(teaspoon)/gi,
				'Tsp.'
			);
			// formValues.ingredients = formValues.ingredients.replace(
			// 	/\d+/g,
